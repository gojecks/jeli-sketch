/**
 * 
 * @param {*} definition 
 * {
 *  pickerImage: string;
 *  showControls: boolean;
 *  height: number;
 *  width: number;
 * }
 */
function jColorPicker() {
    var options = {
            total: 1,
            gradientPattern: ['0', '0.5', '1.0'],
            width: 300,
            height: 300
        },
        board,
        _this = this,
        colorObj = {
            hex: [],
            rgb: [],
            gradient: null
        },
        _callback = function() {};

    this.isHiddenState = false;
    this.cleanup = function() {
        // remove the element 
        board.canvas.width = 0;
        board.canvas.height = 0;
        //this.parentNode.removeChild(this);
        board.destroyState(true);
    };

    this.open = function(definition, dropZone, cb) {
        options = Object.assign({}, options, definition);
        _callback = cb || _callback;
        /**
         * check if the pickerElent has been created the first time
         * show the element instead of creating a new one
         */
        if (this.isHiddenState) {
            this.isHiddenState = false;
            this.pickerElement.show();
            return;
        }

        this.pickerElement = new Modal(dropZone || document.body, {
            onOverlayClick: function() {
                _this.close();
            }
        });

        this.pickerElement
            .addContent('<div id="colorPicker">\
        <div class="controls"><div><label>R</label> <input type="text" id="rVal" /></div> \
        <div><label>G</label> <input type="text" id="gVal" /></div> \
        <div><label>B</label> <input type="text" id="bVal" /></div> \
        <div><label>RGB</label> <input type="text" id="rgbVal" /></div> \
        <div><label>HEX</label> <input type="text" id="hexVal" /></div><div class="preview"></div></div></div>')
            .addStyle('#picker{cursor:crosshair;float:left;margin:10px;border:0}.controls{float:right;margin:10px}.controls>div{border:1px solid #2f2f2f;margin-bottom:5px;overflow:hidden;padding:5px}\
        .controls label{float:left}.controls>div input{background-color:#121212;border:1px solid #2f2f2f;color:#ddd;float:right;font-size:10px;height:14px;margin-left:6px;text-align:center;text-transform:uppercase;width:75px}\
        .preview {border-radius:3px;box-shadow:2px 2px 2px #444;} .preview span{cursor:pointer;height:15px;width:15px; display:inline-block; margin-right: 3px;}');

        // extend the definition
        var pickerContainer = this.pickerElement.querySelector('div#colorPicker'),
            previewContainer = this.pickerElement.querySelector('div.preview');
        board = new Sketch({
                width: options.width || 300,
                height: options.height || 300,
                id: "picker"
            }, pickerContainer),
            img = "data:image/png;base64,";

        // load image
        Sketch.image.call(board, {
            width: options.width,
            height: options.height,
            img: options.img || img,
            toCanvas: true
        }, false);

        /**
         * initialize board
         */
        board.init(function() {
            board.stateManager.setDrawingState("picker", {
                down: _pickColor,
                move: function() {
                    // do nothing
                },
                end: function() {
                    if (options.total === colorObj.hex.length) {
                        _callback(colorObj);
                        _this.close();
                    }
                }
            }, true);

            /**
             * Pick Color
             * @param {*} x 
             * @param {*} y 
             */
            function _pickColor(x, y) {
                var imageData = board.getImageData(x, y, 1, 1),
                    pixel = imageData.data;
                // update controls
                setControlInputValue('#rVal', pixel[0]);
                setControlInputValue('#gVal', pixel[1]);
                setControlInputValue('#bVal', pixel[2]);
                setControlInputValue('#rgbVal', pixel[0] + ',' + pixel[1] + ',' + pixel[2]);
                var dColor = pixel[2] + 256 * pixel[1] + 65536 * pixel[0],
                    rgb = "rgb(" + pixel[0] + ", " + pixel[1] + ", " + pixel[2] + ")",
                    hex = ('0000' + dColor.toString(16)).substr(-6);

                // update preview color
                var selectedColor = document.createElement('span');
                selectedColor.style.backgroundColor = rgb;
                previewContainer.appendChild(selectedColor);
                colorObj.rgb.push(rgb);
                colorObj.hex.push(hex);
                selectedColor = null;
                generateGradient();
                setControlInputValue('#hexVal', '#' + hex);
            }
        });

        /**
         * 
         * @param {*} field 
         * @param {*} value 
         */
        function setControlInputValue(field, value) {
            pickerContainer.querySelector(field).value = value;
        }

        function generateGradient() {
            if (colorObj.rgb.length > 1) {
                var len = colorObj.rgb.length;
                colorObj.gradient = colorObj.rgb.map(function(color, idx) {
                    return {
                        point: idx / len,
                        color: color
                    }
                });
            }
        }


        this.pickerElement.init({
            backgroundColor: "#222",
            borderRadius: "5px 5px 5px 5px",
            boxShadow: "2px 2px 2px #444",
            color: "#fff",
            fontSize: "12px",
            padding: "0px"
        });
    };

    this.close = function() {
        this.isHiddenState = !this.isHiddenState;
        this.pickerElement.hide();
        colorObj = {
            hex: [],
            rgb: []
        };
    };

    this.rgb = function() {
        var color = 'rgb(';
        for (var i = 0; i < 3; i++) {
            color += Math.floor(Math.random() * 255) + ',';
        }
        return color.replace(/\,$/, ')');
    };
}

/**
 * Attach the colorPicker to the sketch
 */
Sketch.colorPicker = new jColorPicker();
/**
 * hidden State
 */
Sketch.colorPicker.isHiddenState = false;